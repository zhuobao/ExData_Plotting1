...
#   ppath: csv file path
#   filename: csv file name
plot3 <- function(gpath=".", gfname="plot3.png", ppath=".", filename="household_power_consumption.txt"){
    ...
    # make chart contains 3 color lines
    xlabtxt=""          # clear X axis label
    ylabtxt="Energy sub metering"       #customize Y axis label
    with(ppdata, plot(Time, Sub_metering_1, type="l", xlab=xlabtxt, ylab=ylabtxt))
    with(ppdata, lines(Time, Sub_metering_2, type="l", col="red"))  # add red line
    with(ppdata, lines(Time, Sub_metering_3, type="l", col="blue")) # add blue line
